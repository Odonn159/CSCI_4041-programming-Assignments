...
def countingSortHouse(array):
    maxval = 3
    map = {"Eagletalon": 0, "Lannister":1, "Pufflehuff":2, "SNAKES":3}
    size = len(array)
    output = [0] * (size+1)
    count = [0] * (maxval+1)
    for i in range(0, size):
        count[map[array[i].house]] += 1
    for i in range(1, maxval+1):
        count[i] += count[i - 1]
    i = size - 1
    while i != -1:
        output[count[map[array[i].house]] - 1] = array[i]
        count[map[array[i].house]] -= 1
        i -= 1
    return output[:-1]
def countingSortYear(array):
    maxval = 8
    size = len(array)
    output = [0] * (size + 1)
    count = [0] * (maxval + 1)
    for i in range(0, size):
        count[array[i].year] += 1
    for i in range(1, maxval + 1):
        count[i] += count[i - 1]
    i = size - 1
    while i != -1:
        output[count[array[i].year] - 1] = array[i]
        count[array[i].year] -= 1
        i -= 1
    return output[:-1]
...
def SortStudents(studentList):
    ...
    #You will be returning the output list rather than passing it in as an argument.
    output = countingSortYear(studentList)
    output = countingSortHouse(output)
    return output
