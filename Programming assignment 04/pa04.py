...
def maxheapify(A, index):
    ...
    R = (index + 1) * 2
    if L <= len(A) - 1:
        if (A[L].priority > A[index].priority):
            largest = L
        else:
            largest = index
    else:
...
def heapincreasekey(A, i, key):
    if key < A[i].priority:
        return None
    A[i].priority = key
    while (i > 0 and A[((i + 1) // 2) - 1].priority < A[i].priority):
        A[i], A[(i + 1) // 2 - 1] = A[(i + 1) // 2 - 1], A[i]
        i = (i + 1) // 2 - 1


def insert_task(A, task):
    A.append(task)
    heapincreasekey(A, len(A) - 1, task.priority)
...
def extract_max_priority_task(task_heap):
    if len(task_heap)==0:
        return None
    task_heap[0], task_heap[len(task_heap) - 1] = task_heap[len(task_heap) - 1], task_heap[0]
    returnval = task_heap.pop()
    maxheapify(task_heap, 0)
    return returnval
...
def increase_task_priority(task_heap, i, new_priority):
    return heapincreasekey(task_heap, i, new_priority)
