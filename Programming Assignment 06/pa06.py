...
def database_search(database,field,value):
    returnlist = []
    if field == "shape":
        for element in database.shape_table[hash_function(value)]:
            if element.shape == value:
                returnlist.append(element)
    elif field == "color":
        for element in database.color_table[hash_function(value)]:
            if element.color == value:
                returnlist.append(element)
    else:
        for element in  database.name_table[hash_function(value)]:
...
def database_delete(database,field,value):
    list = database_search(database,field,value)
    print(list)
    for robot in list:
        database.shape_table[hash_function(robot.shape)].remove(robot)
        database.color_table[hash_function(robot.color)].remove(robot)
        database.name_table[hash_function(robot.name)].remove(robot)
