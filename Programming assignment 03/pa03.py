import traceback
import math
def loopbottom(Master, startval, endof1st, endoflast):
    n1 =endoflast-startval
    A = Master[startval:endof1st].copy()
    B=Master[endof1st: endoflast].copy()
    A.append(-math.inf)
    B.append(-math.inf)
    i = 0
    j = 0
    for k in range(startval,endoflast): # Theta of N (N = endoflast-startval)
        if A[i] > B[j]:
            Master[k] = A[i]
            i += 1
        else:
...
def merge_sort(A):
    z=0
    i=1
    while(i<=len(A)): #Theta of Log(N)
        x=0
        while((x+2*i)<=len(A)): #Theta of N/i
            loopbottom(A,x,x+i,x+2*i) #Theta of i (because i will be the length of the subarrays)
            x+=2*i
            z=x
        i*=2
    loopbottom(A,0,z,len(A))
    #Total Theta of NlogN
